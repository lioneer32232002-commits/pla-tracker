...
- **分帶色＝`_PAI_COLORS`，一組常數供網站／圖卡／日報信／Canva 共用**，改一處全部跟著改。
  2026-08-13 使用者定案「熱度斜坡」：低＝中性灰藍、常態＝暖金、偏高＝黃、高＝橘、極高＝紅。
  被否決且**不要再繞回去**的兩案：(1) 低帶用綠——綠會被讀成「安全」，但零架次只代表
  當天沒偵獲；(2) 全部用紅色深淺——平靜日也是紅的，等於每天都在示警。
  淺色那組不是把深色那組調暗，而是各自挑到「對白底對比 ≈5.0」的同一條線上
  （分帶靠色相區分、不靠深淺），因為這組色同時當文字色用（大數字與分帶名）。
  改色後要重量一次對比：五個分帶對卡片底色都要 ≥4.5。
...
- **指數目前出現在四個地方，改計分邏輯要四個一起看**：
  1. 首頁指數卡（`pai_section_html`）＋ 近 60 個發布日的走勢 sparkline
     （`pai_spark_svg`）。sparkline 是 **SVG 且內部一個字都沒有**（數字都在外面
     的 HTML），所以不碰字型、不違反鐵律 7；同理於月統計頁的純 CSS 日曆。
     `preserveAspectRatio="none"` 會把線條拉粗，所有描邊都必須帶
     `vector-effect="non-scaling-stroke"`。
  2. 月統計頁日曆的格子著色（見下一節）。
  3. `card.html` 分享圖卡：抬頭下方那一行（標籤＋分數＋長條＋分帶）**取代了原本
     的裝飾分隔線**，佔高相同。三個大數字字高 116px、基線 336，字頂落在 y≈255，
     所以這一行必須收在 y<240；版面高度是圖卡的硬約束，不要為了加東西改地圖高度
     （`MH` 一縮，取景就會撞到 `MW/9.0` 的經度視野下限、露出 geo 裁切直邊）。
  4. `send_daily_email.py`：對比列多一列（帶分帶色）、主旨帶「· 強度 NN（分帶）」、
     餵給模型的 summary 有 `activity_index`。該檔**直接 import build_site 的
     `pai_score`**，不要在那邊另寫一份計分。prompt 明文要求貼文引用指數時必須寫
     「本站自算」、禁用「警戒／警報」字眼。
...
- **月統計頁的每日強度日曆（2026-07-30 起；2026-08-13 改依指數著色）**：
  `_monthly_heatmap_html`，純 HTML/CSS grid（不用 Chart.js），列＝月、欄＝該月
  1→31 日，覆蓋全部歷史。三種「空白格」**必須維持區分**：`e`＝該月無此日／未來日、
  `n`＝有此日但國防部未發布、`v0`＝零架次。合併任兩者就是用視覺造假
  （目前 v0 有 42 天、n 只有 1 天）。validate.py 會檢查 `hm-c n`、`hm-c v0`、
  `hm-c v5` 都存在，合併或退回舊著色會被攔下。
  格子深淺 `v1`–`v5`＝**活動強度指數的五個分帶**（原本是 `_HM_BUCKETS` 的架次分桶，
  已刪除）。零架次 `v0` 不併進最低帶——那是一個具體事實，不是「分數低」。
  **色階規則**：單一色相、亮度單調（深色主題遞增 38→68→105→148→181→200，
  淺色遞減 229→212→188→153→112→69）。不要套指數卡那組五色相（綠/灰/黃/橘/紅）：
  8 個月 ×31 格的密集網格用五種色相只會變成雜訊；而且 `v5` 若用紅橘系，會蓋掉格子
  右下角的紅色「越線」標記。淺色主題最深的兩格（v4/v5）另有亮紅點覆寫，
  底色再調深要跟著延伸。
...
- [2026-09-12] 日報信改版（使用者要求「版型更好閱讀」＋「只留情勢摘要跟貼文建議，預算議題不要提」）：`send_daily_email.py` 的 prompt 從五節（情勢摘要／因果鏈／美日智庫動向／事實查核／Threads 貼文建議）縮成**兩節**，貼文不再並置國防預算／立法院／在野黨議題，每則上限 500→300 字；信件版型改成結構化——`kpi_html` 五格數字卡（table，Gmail 手機版不吃 grid）、`parse_analysis` 依「【情勢摘要】／【Threads 貼文建議】／---」拆段、兩則草稿各一張白底卡；近 48 小時新聞附錄**已從信裡拿掉**（新聞仍抓、只餵模型當軍事背景）。解析失敗時整段原樣顯示，不會寄不出去。零架次日 `cross_rate` 為空曾顯示 `nan%`，已改「—」。**不要把預算／在野黨並置角度加回貼文**，那是使用者明確退掉的。本機驗收：不呼叫 API 直接呼叫 `render_email_html()` 出 HTML 看版面即可。
- [2026-09-06] 症狀：當日三班 CI 全紅、網站停在 9/05，錯誤與 8/14 一字不差
  （validate「en 頁含中文字元」）。根因：公告的空域寫法多了一個量詞——
  「逾越中線進入北部、中部、西南及東部空域**共**13架次」。fetch 端的 `_ZONE_COUNT_RE`
  與 build 端 Pattern 4 都要求「空域」後**直接**接數字，8/14 補的特例只認得沒有量詞的
  「N架次」，多一個「共」字整組不匹配 → 英文版殘留中文 → validate 擋下 commit。
  修法：把尾巴的認定收斂成 `build_site.ZONE_COUNT_TAIL` 單一常數（括號有無、量詞
  共／計／合計／總計、單位架／架次一次涵蓋），`fetch_and_update.py` 直接 import 共用，
  不再兩邊各寫一份；`_extract_crossing_regions` 也改成先剝尾巴再做「空域$」錨定。
  **上位規則（取代逐次補特例）：同一個欄位的格式認定只能有一份，寫成共用常數／函式；
  發現漏判時修那一份，不要在第二個檔案補第二個特例。** 判斷「該收斂了」的訊號是
  同一個症狀出現第二次——本例第一次（8/14）補特例、第二次（9/06）就又漏。
  歷史案例：2026-08-13（空域整欄留空）、2026-08-14（尾巴無括號）兩條詳見下方。
  附帶未處理：`_translate_airspace_block` 內部仍只認「架次」不認「架」，所以
  2026-09-02 的 `北部空域(1架)` 英文版少了 `(1 sortie)`；不影響驗證，要改需動五個
  Format 分支。
- [2026-08-14] 症狀：當日三班 CI 全紅、網站停在 8/13；log 顯示擷取成功、build 成功，
  死在 validate「en 頁含中文字元」。根因：模型回傳的空域字串沒括號
  （「逾越中線進入中部、西南及東部空域6架次」），而 8/13 加的 `zone_phrase_from_text`
  正規化**只在模型留空時啟動**，有值就原樣入 CSV；英文翻譯的正則以「空域」結尾錨定，
  尾巴多了「6架次」整組不匹配。修法兩層：fetch 端不論字串來源一律過 `_ZONE_COUNT_RE`
  正規化；build 端 Pattern 4 允許結尾無括號的「N架次」。
  規則：**加保險絲時要涵蓋「值存在但格式異常」的路徑，不能只接「值缺失」**——
  同一格式問題會從多個入口進來（模型輸出、內文擷取、人工回填）。
- [2026-08-13] 症狀：8/12、8/13 連兩天架次與越線都對，但 `special_event` 空白，
  首頁地圖、分享圖卡、Canva 圖卡三處的空域一起消失（CI 全綠、validate 也過，
  因為空域欄位允許留空）。根因：**8/11 15:36 那顆模型升級 commit（`41e3b49`，
  Opus 4.6 → `claude-opus-5`）改變了判讀行為**——8/11 的資料是 04:00 用舊模型抓的、
  有空域；8/12 起全部由 Opus 5 抓，圖上沒標空域名稱它就誠實留空，
  舊模型則會從航跡與 ①② 標註推出區域。同時**擷取器有圖的日子只餵圖、完全不讀公告內文**，
  而空域名稱穩定寫在內文的括號裡（「偵獲共機18架次（逾越中線進入北部、中部、
  西南及東部空域15架次）」），所以圖上沒標就整組漏掉。修法兩層：有圖的日子把內文
  一起餵給模型（內文 140–160 字，相對圖片 ~1,180 token 幾乎不加成本），再加一道
  **不依賴模型**的正則保險（`zone_phrase_from_text()`，抓含「空域」的那組括號）。
  **規則：升級 CI 的模型後，必須拿升級前後各兩三天的擷取結果逐欄比對**——
  「跑得起來、沒噴錯」不等於沒退化，欄位允許留空的更會靜默漏資料。
  另一條上位規則：**任何欄位若只有單一來源（這裡是航跡圖），就要有第二來源或
  純字串的保險絲**，不要把可用文字取得的資訊押在模型判讀圖片上。
