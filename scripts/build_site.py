...
// Zone polygons approximate MND bulletin areas
// Zone polygons match MND bulletin grid — all kept outside Taiwan's 12nm line
// 北部空域 — northern strait + NW Pacific, above Taiwan's north tip
if(ZONES.n){
  L.polygon([[25.5,120.5],[26.5,120.5],[26.5,122.5],[25.5,122.0]],zS).addTo(map);
  zoneLabel([26.1,121.2],'北部空域');
}
// 西南部空域 — per MND zone ②: SW quadrant, east edge ~120°E at 23°N, widens south
if(ZONES.sw){
  L.polygon([[21.0,117.0],[23.0,117.0],[23.0,119.8],[21.0,120.8]],zS).addTo(map);
  zoneLabel([21.6,117.8],'西南部空域');
}
...
// 東北部空域 — per MND zone ①: north/NE of Taiwan, 25.5-26.5°N, 121-122.5°E
if(ZONES.ne){
  L.polygon([[25.5,121.0],[26.5,121.0],[26.5,122.5],[25.5,122.0]],zS).addTo(map);
  zoneLabel([26.0,121.5],'東北部空域');
}
// 南部空域 — south of Taiwan's southern tip, Pacific + Bashi Channel side
if(ZONES.s){
...
info.onAdd=function(){
  ...
  var n1='text-align:right;padding-right:3px;white-space:nowrap;color:#f5c842';
  var n2='text-align:right;padding-right:3px;white-space:nowrap;color:#e05555';
  var mlRow=ML>0
    ?'<tr><td style="'+n2+'">\\u2715 \\u9032 '+ML+'</td><td style="white-space:nowrap;color:#e05555">逾中線</td></tr>'
    :'<tr><td colspan="2" style="color:#2a4a60;white-space:nowrap">\\u2500 未逾中線</td></tr>';
  d.innerHTML=
    '<table style="border-spacing:0;font-size:.7rem"><tbody>'+
    '<tr><td style="'+n1+'">&#9992; '+AC+'</td><td style="white-space:nowrap;color:#f5c842">架次</td></tr>'+
    '<tr><td style="'+n2+'">&#9875; '+SH+'</td><td style="white-space:nowrap;color:#e05555">艘艦艇</td></tr>'+
    mlRow+
    '</tbody></table>';
  return d;
};
