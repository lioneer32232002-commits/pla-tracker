...
# ── records / monthly / about 頁結構化資料（WebPage + BreadcrumbList）──────────

def page_jsonld(lang, s, page_name):
    """WebPage + BreadcrumbList head_extra for records / monthly / about pages."""
    pfx   = '/en' if lang == 'en' else ''
    home  = canon_url('/index.html', pfx)
    page  = canon_url(f'/{page_name}.html', pfx)
    label = s[f'nav_{page_name}']
    webpage = {
        "@context": "https://schema.org",
        "@type": "WebPage",
        "name": s['page_titles'][page_name],
        "description": s['meta_descs'][page_name],
        "url": page,
        "inLanguage": 'en' if lang == 'en' else 'zh-Hant',
        "isPartOf": {"@type": "WebSite", "name": s['site_title'], "url": home},
    }
    breadcrumb = {
        "@context": "https://schema.org",
        "@type": "BreadcrumbList",
        "itemListElement": [
            {"@type": "ListItem", "position": 1,
             "name": 'Home' if lang == 'en' else '首頁', "item": home},
            {"@type": "ListItem", "position": 2, "name": label, "item": page},
        ],
    }
    return ''.join('\n<script type="application/ld+json">'
                   + json.dumps(b, ensure_ascii=False) + '</script>'
                   for b in (webpage, breadcrumb))
...
def build_records(df, lang, out_dir, s):
    ...
    head = make_head(lang, 'records', s, head_extra=page_jsonld(lang, s, 'records'))
    html = f"""{head}
<body>
<header class="site-header">
  <div class="header-inner">
...
def build_monthly(df, lang, out_dir, s):
    ...
    head = make_head(lang, 'monthly', s, head_extra=page_jsonld(lang, s, 'monthly'))
    html = f"""{head}
<body>
<header class="site-header">
  <div class="header-inner">
...
{pai_method_html(lang, s)}
    ...
    head = make_head(lang, 'about', s, head_extra=page_jsonld(lang, s, 'about'))
    html = f"""{head}
<body>
<header class="site-header">
  <div class="header-inner">
...
function tw_(t,sz,w,sp,fam){ff(sz,w,sp,fam);var v=x.measureText(t).width;if(LS)x.letterSpacing='0px';return v;}
...
txt(D.lbl.tw,pX(120.96),pY(24.28),32,'#f2f7fa',700,'left',3);
txt(D.lbl.ph,pX(119.24),pY(23.36),20,'#a9b7c0',500,'left',1);
txt(D.lbl.km,pX(118.06),pY(24.34),20,'#a9b7c0',500,'left',1);
txt(D.lbl.mz,pX(119.62),pY(26.02),20,'#a9b7c0',500,'left',1);
...
paint();
// canvas 用到的網頁字型**不會**被瀏覽器自動下載——字型是依 DOM 的使用情況才載，
