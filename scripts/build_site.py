...
// 北部空域 — northern strait + NW Pacific, north of Keelung
if(ZONES.n){
  L.polygon([[25.0,120.0],[26.5,120.5],[26.5,122.5],[25.0,122.0]],zS).addTo(map);
  zoneLabel([25.8,121.0],'北部空域');
}
// 西南部空域 — SW quadrant per MND zone ②: 21-23°N, 117-121°E
if(ZONES.sw){
  L.polygon([[21.0,117.0],[23.0,117.0],[23.0,121.0],[21.0,121.0]],zS).addTo(map);
  zoneLabel([21.5,118.0],'西南部空域');
}
// 東部空域 — east of Taiwan, Pacific side
if(ZONES.e){
  L.polygon([[22.0,121.8],[24.5,121.8],[24.5,123.5],[22.0,123.5]],zS).addTo(map);
  zoneLabel([22.8,122.3],'東部空域');
}
// 東北部空域 — per MND zone ①: northern strait + NE, 25.3-26.5°N, 120.5-122.5°E
if(ZONES.ne){
  L.polygon([[25.3,120.5],[26.5,121.0],[26.5,122.5],[25.0,122.0]],zS).addTo(map);
