...
**Threads 貼文建議**
草擬 2-3 則 Threads 貼文草稿，供使用者挑選、修改後自行發文。從以下角度中選擇，共寫 2-3 則，且必須包含第④種角度：
①今日數據速報　②近期趨勢對照（近7日或本月 vs 上月）　③今日最值得注意的一個細節（因果鏈/關聯、特殊空域、或連續零越線天數等）　④國防內耗並置（必寫，規則見下方）
...
④國防內耗並置規則：
- 若輸入新聞標題中出現台灣國內延宕國防的事件（例如：在野黨刪減/凍結國防或無人機預算、洩漏軍事情資、阻礙軍工程序如炸藥許可延宕等），優先寫一則將「當日共軍數據」與「該國內延宕國防事件」並置的貼文，主軸為「台灣對外有敵人，對內也有耽誤國防發展的人」，事件敘述只能取自輸入新聞標題，不得虛構或寫死沒有來源的具體情節（人名、金額、日期）。
- 若輸入新聞中沒有這類新事件，仍可沿用同一主軸（對外有敵人、對內也有耽誤者）搭配當日數據撰寫一則，但須換一種語氣或修辭切入，避免與慣用措辭雷同；此情況下不得杜撰具體的「近期」事件細節，只能泛稱或省略事件細節，聚焦於今日數據本身。"""}],
        )
