...
def main():
    ...
    # 地圖投影：由元素實際大小反推（元素被縮放時，viewBox 內的座標同比例縮放）
    sx, sy = args.width / MAP_VB_W, args.height / MAP_VB_H
    ...
    def page_x(lon):
        return args.left + (lon - lon0) * k * sx

    def page_y(lat):
        return args.top + (latt - lat) * ky * sy

    zones_on = zones_from_special(row['special_event'])
    zones, labels = [], []
    for zk, on in zones_on.items():
        if not on:
            continue
        poly, focus, lp = ZP[zk]
